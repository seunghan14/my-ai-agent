...
.stApp p,.stApp span,.stApp label,.stApp div{color:#c8c8c8}
h1,h2,h3,h4{color:#e8e8e8 !important}
.stTextInput input,.stTextArea textarea,.stNumberInput input{background:#2a2a2a !important;color:#e0e0e0 !important;border-color:#444 !important}
...
# ===== NOTES =====
    ...
    if st.session_state.editing_note:
        ...
        sel_sum=pc1.selectbox("Summary 프롬프트",sum_prompts,key="sel_sum")
        sel_exp=pc2.selectbox("Expand 프롬프트",exp_prompts,key="sel_exp")
...
# ===== AI CONTENT =====
    ...
    kw=st.text_input("🔑 Keywords"); img=st.file_uploader("📷 Image",type=["png","jpg","jpeg"],key="ci")
    ...
    if st.button("✨ Generate",type="primary",use_container_width=True):
        if topic:
            with st.spinner("Generating..."):
                ...
                if result:
                    sc1,sc2=st.columns(2)
                    if sc1.button("📝 Save",key="sc_note"):
...
# ===== ECONOMY =====
    ...
    with tabs[1]:
        ...
        if sub=="Expenses":
            ...
            if DB:
                for e in get_expenses(uid,now_kst().strftime("%Y-%m"))[:20]: st.markdown(f"- {e.get('expense_date','')} | {e.get('category','')} | {e.get('amount',0):,}₩")
        elif sub=="Income":
            ...
            uploaded=st.file_uploader("Excel/CSV",type=["csv","xlsx","txt"],key="eu"); text_in=st.text_area("Or paste",height=100,key="eti")
    ...
    with tabs[2]:
        if st.button("🔄 Market Info",type="primary"):
...
# ===== POMODORO =====
    ...
    timer_html=f"""<style>.pc{{text-align:center;padding:1.2rem;font-family:-apple-system,sans-serif}}.td{{font-size:5rem;font-weight:800;color:#ef4444;font-family:'Courier New',monospace;letter-spacing:4px;margin:0.5rem 0}}.td.brk{{color:#22c55e}}.ts{{font-size:1rem;color:#666;margin-bottom:0.8rem;min-height:1.4rem}}.prog{{width:75%;max-width:380px;height:8px;background:#e5e7eb;border-radius:4px;margin:0 auto 1rem;overflow:hidden}}.pb{{height:100%;background:linear-gradient(90deg,#ef4444,#f97316);border-radius:4px;transition:width 1s linear}}.pb.brk{{background:linear-gradient(90deg,#22c55e,#14b8a6)}}.br{{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}}.btn{{padding:0.6rem 1.4rem;font-size:0.9rem;border-radius:8px;border:none;cursor:pointer;font-weight:600}}.bs{{background:#ef4444;color:#fff}}.bb{{background:#22c55e;color:#fff}}.br2{{background:#6b7280;color:#fff}}.sc{{font-size:0.85rem;color:#888;margin-top:0.7rem}}</style>
<div class="pc"><div class="td" id="td">{focus_min:02d}:00</div><div class="ts" id="ts">집중할 준비가 되셨나요? 🍅</div><div class="prog"><div class="pb" id="pb" style="width:100%"></div></div><div class="br"><button class="btn bs" id="sb" onclick="toggle()">▶️ 시작</button><button class="btn bb" onclick="startBreak()">☕ 휴식</button><button class="btn br2" onclick="reset()">🔄 리셋</button></div><div class="sc" id="sc">오늘 완료: 0 🍅</div></div>
<script>const F={focus_min}*60,B={break_min}*60;let left=F,total=F,run=false,brk=false,iv=null,sess=0;function fmt(t){{return String(Math.floor(t/60)).padStart(2,'0')+':'+String(t%60).padStart(2,'0')}}function ui(){{document.getElementById('td').textContent=fmt(left);document.getElementById('td').className='td'+(brk?' brk':'');document.getElementById('pb').style.width=(left/total*100)+'%';document.getElementById('pb').className='pb'+(brk?' brk':'')}}function beep(){{try{{const a=new AudioContext(),o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.type='sine';o.frequency.setValueAtTime(880,a.currentTime);o.frequency.setValueAtTime(660,a.currentTime+0.2);o.frequency.setValueAtTime(880,a.currentTime+0.4);g.gain.setValueAtTime(0.3,a.currentTime);g.gain.exponentialRampToValueAtTime(0.01,a.currentTime+0.7);o.start();o.stop(a.currentTime+0.7)}}catch(e){{}}}}function toggle(){{if(run){{clearInterval(iv);run=false;document.getElementById('sb').textContent='▶️ 재개';document.getElementById('ts').textContent='⏸️ 일시정지'}}else{{run=true;document.getElementById('sb').textContent='⏸️ 일시정지';document.getElementById('ts').textContent=brk?'☕ 휴식 중...':'🍅 집중 중!';iv=setInterval(()=>{{if(left>0){{left--;ui()}}else{{clearInterval(iv);run=false;document.getElementById('sb').textContent='▶️ 시작';if(!brk){{sess++;document.getElementById('sc').textContent='오늘 완료: '+sess+' 🍅';document.getElementById('ts').textContent='✅ 완료! 아래 버튼으로 기록하세요 👇';beep()}}else{{document.getElementById('ts').textContent='💪 휴식 완료! 다시 집중해봐요';brk=false;left=F;total=F;ui()}}}}}}),1000)}}}}function startBreak(){{clearInterval(iv);run=false;brk=true;left=B;total=B;document.getElementById('sb').textContent='▶️ 시작';document.getElementById('ts').textContent='☕ 휴식 준비';ui()}}function reset(){{clearInterval(iv);run=false;brk=false;left=F;total=F;document.getElementById('sb').textContent='▶️ 시작';document.getElementById('ts').textContent='집중할 준비가 되셨나요? 🍅';ui()}}</script>"""
    ...
    if DB:
        ...
        if logs:
            ...
            col_s1.metric("완료 세션",f"{len(complete_logs)} 🍅"); col_s2.metric("총 집중",f"{sum(l.get('duration_minutes',25) for l in complete_logs)}분"); col_s3.metric("완주율",f"{int(len(complete_logs)/len(logs)*100) if logs else 0}%")
...
# ===== SETTINGS =====
    ...
    with tabs_s[0]:
        ...
        new_pw=st.text_input("새 비밀번호",type="password",key="new_pw"); new_pw2=st.text_input("확인",type="password",key="new_pw2")
    ...
    with tabs_s[3]:
        st.markdown("### 🎯 목표 & 습관 관리")
        hc1,hc2,hc3=st.columns([2,1,1])
        hn=hc1.text_input("이름"); hi=hc2.text_input("아이콘",value="✅",key="hi"); htype=hc3.selectbox("타입",["check","numeric"],format_func=lambda x:{"check":"✅ 체크형","numeric":"📊 수치형"}[x],key="htype")
        htarget=1.0; hunit=""
        if htype=="numeric":
            hc4,hc5=st.columns(2); htarget=hc4.number_input("목표값",min_value=0.1,value=1.0,step=0.5,key="htarget"); hunit=hc5.text_input("단위",placeholder="km, 잔, 분...",key="hunit")
    ...
    with tabs_s[6]:
        st.caption("노트 Summary/Expand 및 Pomodoro 분석에 사용할 커스텀 프롬프트")
