...
# === GOOGLE TOKENS ===
def get_google_tokens(uid):
    q = _q("profiles")
    if not q: return None
    try:
        r = q.select("google_access_token,google_refresh_token,google_token_expiry").eq("id",uid).execute()
        if r.data:
            d = r.data[0]
            return {"access_token":d.get("google_access_token"),"refresh_token":d.get("google_refresh_token"),"token_expiry":d.get("google_token_expiry")}
    except: pass
    return None

def save_google_tokens(uid, access_token, refresh_token, expiry):
    return update_profile(uid, google_access_token=access_token, google_refresh_token=refresh_token, google_token_expiry=expiry)

def clear_google_tokens(uid):
    return update_profile(uid, google_access_token=None, google_refresh_token=None, google_token_expiry=None)
...
def get_all_links(uid):
    notes = get_notes(uid); nids = [n["id"] for n in notes]
    q = _q("note_links")
...
def create_event(uid, title, start, end=None, desc="", color_label="blue", gcal_id=None, source="local"):
    q = _q("calendar_events")
    if not q: return None
    try:
        d = {"user_id":uid,"title":title,"start_time":start.isoformat(),"description":desc,"color_label":color_label,"source":source}
        if end: d["end_time"] = end.isoformat()
        if gcal_id: d["gcal_id"] = gcal_id
        return q.insert(d).execute().data[0]
    except: return None
...
def get_expenses(uid, month=None):
    ...
    try:
        ...
        if month:
            ...
            start=f"{month}-01"; end=f"{y+1}-01-01" if m==12 else f"{y}-{m+1:02d}-01"
...
def create_habit_v2(uid, name, icon="✅", habit_type="check", target_value=1, unit=""):
    ...
    if not q: return None
    try:
        return q.insert({"user_id":uid,"name":name,"icon":icon,"habit_type":habit_type,"target_value":float(target_value),"unit":unit}).execute().data[0]
    except:
        try: return q.insert({"user_id":uid,"name":name,"icon":icon}).execute().data[0]
        except: return None
...
def toggle_habit(hid, uid, d=None):
    ...
    try:
        ...
        if r.data: q.update({"completed":not r.data[0]["completed"]}).eq("id",r.data[0]["id"]).execute()
        else: q.insert({"habit_id":hid,"user_id":uid,"log_date":ds,"completed":True}).execute()
        return True
    except: return False
...
def log_pomo(uid, dur=25, task="", status="complete", interruptions=0):
    ...
    if not q: return None
    try:
        return q.insert({"user_id":uid,"duration_minutes":dur,"task_name":task,"status":status,"interruptions":interruptions}).execute().data[0]
    except:
